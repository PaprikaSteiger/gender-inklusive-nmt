...
def compare_files(gold_file: Path, trial_file: Path):
    differences = []
    with gold_file.open("r", encoding="utf8") as gold, trial_file.open(
        "r", encoding="utf8"
    ) as trial, open(
        (outfile.parent / "fr_differences.txt"), "w", encoding="utf8"
    ) as out:
...
if __name__ == "__main__":
    ...
    det_noun_adj = [
        ...
        {
            "POS": "NOUN",
            "DEP": {"INTERSECTS": ["nsubj", "nsubj:pass", "obj", "obl:arg"]},
        },
        {"POS": "ADJ", "OP": "*"},
        {"POS": "CONJ", "OP": "?"},
        {"IS_PUNCT": True, "OP": "?"},
    ...
    clean_annotated_file(
        infile=(DIR / "fr_test_set_annotated.txt"),
        outfile=(DIR / "fr_test_set_annotated_spacy_clean.txt"),
    )
